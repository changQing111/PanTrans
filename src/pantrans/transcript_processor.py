...
def extract_exon_coord_splice_site(bam_path, cluster_dic):
    ...
    for read in bam:
        ...
        query_id = ".".join(query.split(".")[:-1])
        if query_id not in gene_li:
            continue

        # 初始化
        trans_splice_site_dic[query] = []
        exon_coord_dic[query] = []

        current_pos = read.reference_start + 1  # BAM坐标是0-based，这里转为1-based
        exon_start = current_pos

        for op, length in read.cigartuples:
            if op in (0, 7, 8):  # M, =, X → 匹配到参考基因组
                current_pos += length
            elif op == 3:  # N → intron (跳过的区域，剪切位点)
                exon_end = current_pos
                exon_coord_dic[query].append((exon_start, exon_end))  # 保存外显子区间

                splice_start = current_pos
                splice_end = current_pos + length
                trans_splice_site_dic[query].append((splice_start, splice_end))  # 保存剪切位点

                current_pos = splice_end
                exon_start = current_pos  # 下一个外显子起点
            else:
                # 其他CIGAR操作（插入、缺失等），这里简单跳过或可扩展
                pass

        # 最后一个外显子
        exon_end = current_pos
        exon_coord_dic[query].append((exon_start, exon_end))
...
def generate_gtf(gene_splice_site_dic, exon_coord_dic, gene_len_dic, trans_len_dic, gene_strand_dic, gtf_path):
    """生成GTF文件，每个剪切位点模式选择最长的转录本作为代表"""
    gtf_file = open(gtf_path, "w")
    for i, j in gene_splice_site_dic.items():
        gene_id = i
        strand = gene_strand_dic[gene_id]
        gene_len = gene_len_dic[gene_id]
        for index, trans_li in enumerate(j.values()):
            # 过滤掉不在 trans_len_dic 中的转录本
            #valid_trans_li = [trans for trans in trans_li if trans in trans_len_dic]
            #if not valid_trans_li:
            #    continue
            trans_len = [trans_len_dic[trans] for trans in trans_li]
            trans_id = gene_id + "." + str(index + 1)
            anno_col = f'transcript_id "{trans_id}"; gene_id "{gene_id}";'
            # 检查 trans_id 是否在 exon_coord_dic 中
            if trans_id not in exon_coord_dic:
                continue
            exon_coord_li = exon_coord_dic[trans_id]
            if not exon_coord_li:
                continue
            start = str(exon_coord_li[0][0])
            end = str(min(exon_coord_li[-1][-1], gene_len))
            gtf_trans_li = [gene_id, "Pan", "transcript", start, end, ".", strand, ".", anno_col]
            gtf_file.write("\t".join(gtf_trans_li) + "\n")
            for coord in exon_coord_li:
                start_i = coord[0]
                end_i = min(coord[-1], gene_len)
                start = str(start_i)
                end = str(end_i)
                gtf_exon_li = [gene_id, "Pan", "exon", start, end, ".", strand, ".", anno_col]
...
def transcript_dedup(bam_path, cluster_dic, trans_len_dic, gene_len_dic, gene_strand_dic, gtf_path):
    ...
    generate_gtf(gene_splice_site_dic, exon_coord_dic, gene_len_dic, trans_len_dic, gene_strand_dic, gtf_path)
...
def get_cdna_from_gtf(fasta_path, gtf_path, out_path):
    ...
    for tid, exons in transcripts.items():
        if not exons:
            continue
        strand = exons[0][3]
        exons_sorted = sorted(exons, key=lambda x: x[1], reverse=(strand == "-"))
