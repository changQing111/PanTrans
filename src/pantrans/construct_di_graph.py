...
def find_max_in_degree_nodes(in_degrees, gene_len_dic, refer_name):
    """找到入度最大的节点，若有多个则选长度最长的，若仍有多个则优先含refer_name的"""
    ...
    CS_index_li = [i for i in max_index_li if refer_name in max_nodes[i]]
    if CS_index_li:
        max_node = max_nodes[CS_index_li[0]]
    else:
...
def unit_recursion(G, node_li, gene_len_dic, refer_name):
    ...
    while pending:
        ...
        max_node = find_max_in_degree_nodes(node_degree_dic, gene_len_dic, refer_name)
...
def assign_sccs(sccs, G, gene_len_dic, bed_dic, variety_li, refer_name):
    """
    ...
    for scc in sccs:
        ...
        if len(li) == 1:
            ...
            clusters = unit_recursion(G, li, gene_len_dic, refer_name)
            pre_cluster.extend(clusters)
            for cluster in clusters:
                if len(cluster) == 1:
                    last_clusters.append(cluster)
                else:
                    assigned_cluster = assign_gene_by_chrom(variety_li, cluster, bed_dic)
                    for ac in assigned_cluster:
                        if len(ac) == 1:
                            last_clusters.append(ac)
                        else:
                            last_clusters.extend(unit_recursion(G, ac, gene_len_dic, refer_name))
