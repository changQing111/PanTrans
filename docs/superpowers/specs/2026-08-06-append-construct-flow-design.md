...
This change addresses the observed inflation of append `last.cluster`: genes absent from the append alignment graph must not be recovered from BED records outside the actual append sequence inputs as singleton clusters.
...
pantrans append \
  ...
  --cdna <new.cdna.fasta> \
  --gdna <new.gdna.fasta> \
  --bed <pre-representatives-plus-new-variety.bed> \
...
- Remove the separate `--refer_bed`. `--bed` contains the historical pre-cluster representative genes followed by all genes from the new variety or varieties.
...
The historical pan-genome is intentionally represented only by its pre-cluster representative cDNA and gDNA sequences. Therefore, append can cluster only:

1. historical pre-cluster representative genes present in `--refer_gdna` and `--bed`; and
2. new-variety genes present in the query gDNA.

`--bed` is prepared before append by combining the BED rows for the first gene ID of each historical pre cluster with the BED rows for all new-variety genes. It is used for chromosome assignment, strand lookup, ordering, and final BED output. Historical non-representative genes are deliberately absent because their membership cannot be reconstructed without `--refer_cluster`.

Reference variety prefixes are inferred from this combined BED by subtracting the names supplied through `--name`. These prefixes are passed to representative selection as the historical reference set. For chromosome assignment, all historical prefixes continue to act as the single logical `Refer` group, while each appended variety remains a separate group.
...
4. Read the supplied combined BED directly; append does not accept or concatenate separate reference and query BED files.
...
- append: gene IDs present in both the merged gDNA FASTA and the supplied combined BED.

The append eligible set is the critical invariant. A BED-only ID cannot be recovered as a singleton unless its genomic sequence was actually part of the append comparison. Under a valid input, the reference portion of the BED matches `--refer_gdna`, and the new-variety portion matches the query gDNA.
...
- Fail if reference variety prefixes cannot be inferred from the supplied combined BED.
- Fail if an optional BAM does not exist.
- Report any mismatch between merged gDNA gene IDs and BED gene IDs clearly; these inputs are expected to describe the same historical representatives and new genes.
...
5. A BED-only gene absent from merged gDNA is not recovered as a singleton cluster.
